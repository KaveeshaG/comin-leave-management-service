...
type LeaveRepository interface {
    ...
    CreateLeaveRequest(orgId uuid.UUID, request *domain.LeaveRequest) error
    GetLeaveRequest(id uuid.UUID) (*domain.LeaveRequest, error)
    UpdateLeaveRequest(orgID uuid.UUID, reqID uuid.UUID, request *domain.LeaveRequest, userID uuid.UUID) error
...
func (r *leaveRepository) CreateLeaveRequest(orgId uuid.UUID, request *domain.LeaveRequest) error {
    return r.db.Transaction(func(tx *gorm.DB) error {
...
func (r *leaveRepository) UpdateLeaveRequest(orgID uuid.UUID, reqID uuid.UUID, updatedRequest *domain.LeaveRequest, userID uuid.UUID) error {
    return r.db.Transaction(func(tx *gorm.DB) error {
        existingRequest := &domain.LeaveRequest{}
        if err := tx.First(existingRequest, "id = ?", reqID).Error; err != nil {
            return fmt.Errorf("failed to find leave request with ID %s: %w", reqID, err)
        }
        ...
        if existingRequest.Status == "approved" {
            existingRequest.ApprovedBy = &userID
            existingRequest.ApprovedAt = &time.Time{}
            *existingRequest.ApprovedAt = time.Now()
        } else if existingRequest.Status == "rejected" {
            // Similarly handle rejection
            // existingRequest.RejectedBy = &userID
            // existingRequest.RejectedAt = &time.Time{}
            // *existingRequest.RejectedAt = time.Now()
        }
